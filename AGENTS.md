...
- Use these files to validate and manage:
  ...
  - dependency declarations and environment bootstrap configuration
...
- Use a project-local virtual environment (`.venv`) managed by `uv`.
- Required tooling for repository workflows (lint, format, test, docs) must be declared in project-managed dependencies (not assumed from global installs).
- Keep bootstrap and lock workflow consistent:
  - `environment.yml`: bootstrap prerequisites (for example Python and `uv`)
  - `pyproject.toml`: runtime and development dependency declarations
...
3. Run commands through the project environment (`uv run ...`) rather than global binaries.
...
- Run project commands from environment: `uv run <command>`
...
- dependency declaration updates include corresponding `uv.lock` updates
